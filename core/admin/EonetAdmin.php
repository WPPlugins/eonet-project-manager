...
class EonetAdmin extends Eonet
{
    ...
    public function ajaxSaveSettings() {
        ...
        if($value || $eonet_settings == get_option($option_field)) {
            ...
            $response['title'] = esc_html__('Done, everything is safe !', 'eonet-project-manager');
            $response['content'] = esc_html__('Your settings have been saved successfully.', 'eonet-project-manager');
        } else {
            $response['status'] = 'error';
            $response['title'] = esc_html__('Something went wrong...', 'eonet-project-manager');
            $response['content'] = esc_html__("We've not been able to save your settings, please try again later.", 'eonet-project-manager');
        }
    ...
    public function ajaxResetSettings() {
        ...
        if($value) {
            $response['status'] = 'success';
            $response['title'] = esc_html__('Done, settings have been reset !', 'eonet-project-manager');
            $response['content'] = esc_html__('The page will be refreshed in a few seconds...', 'eonet-project-manager');
        } else {
            $response['status'] = 'error';
            $response['title'] = esc_html__('Something went wrong...', 'eonet-project-manager');
            $response['content'] = esc_html__("We've not been able to reset your settings, please try again later.", 'eonet-project-manager');
        }
    ...
    /**
     * Eonet add menus. It check if the current files are inside a theme or a plugin (this is the core of our products),
     * and it uses the right functions consequentially
     */
    public function adminMenus(){
	    ...
	    if ( false !== strpos( __FILE__, WP_PLUGIN_DIR ) || false !== strpos( __FILE__, WPMU_PLUGIN_DIR ) ) {
		    ...
		    add_menu_page(
			    esc_html__( 'Eonet Dashboard', 'eonet-project-manager' ),
			    esc_html__( 'Eonet','eonet-project-manager' ),
			    'manage_options',
			    'eonet',
    ...
    static public function getThemes() {
        $themes = array(
            array(
                'name' => esc_html__('Woffice Intranet/Extranet theme', 'eonet-project-manager'),
                ...
                'premium' => true,
                'features' => array(
                    esc_html__('Projects Management','eonet-project-manager'),
                    esc_html__('Complete Wiki/Knowledge base','eonet-project-manager'),
                    esc_html__('Members map & directory','eonet-project-manager'),
                    esc_html__('Responsive layout (mobile & tablet)','eonet-project-manager'),
                    esc_html__('Cover picture','eonet-project-manager'),
                    esc_html__('Private / Public pages','eonet-project-manager'),
                    esc_html__('Files Manager','eonet-project-manager'),
                    esc_html__('Events calendar','eonet-project-manager'),
                )
            )
        );
